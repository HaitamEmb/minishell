...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/16 01:59:40 by helmouta          #+#    #+#             */
/*   Updated: 2025/12/16 12:09:34 by isingara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	del_empty_args(t_token **token)
{
	...
	while (tmp->type == WORD || tmp->type == VAR)
	{
		if (tmp->type == VAR && tmp->str[0] == '\0'
			&& tmp->is_var_exist == false)
		{
			tmp = tmp->next;
			if (tmp == (*token)->next)
				(*token) = (*token)->next;
			lst_deltoken(tmp->prev, free_ptr);
		}
		else
			tmp = tmp->next;
	}
